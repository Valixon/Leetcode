public class L_1105 {
    ...
    public static int minHeightShelves(int[][] books, int shelfWidth) {
        ...
        return salomlashish(0, 0, 0, books, shelfWidth);
    }

    private static int salomlashish(int qatorUzunligi, int qatorBalandligi, int index, int[][] books, int shelfWidth) {
        if (index == books.length) return qatorBalandligi;
        if (qatorUzunligi == 0) {
            if (dp[index] == 0) {
                dp[index] = salomlashish(books[index][0], books[index][1], index + 1, books, shelfWidth);
            }
            return dp[index];
        }
        if (qatorUzunligi + books[index][0] > shelfWidth) {
            if (dp[index] == 0) {
                dp[index] = salomlashish(books[index][0], books[index][1], index + 1, books, shelfWidth);
            }
            return qatorBalandligi + dp[index];
        }
        if (dp[index] == 0) {
            dp[index] = salomlashish(books[index][0], books[index][1], index + 1, books, shelfWidth);
        }
        return Math.min(qatorBalandligi + dp[index],
                salomlashish(qatorUzunligi + books[index][0], Math.max(qatorBalandligi, books[index][1]), index + 1, books, shelfWidth));
    }
}
